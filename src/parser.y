...
%code {
  ...
  static inline SrcPos HERE() {
    SrcPos p;
    p.line = yylineno;
    p.col  = token_start_column;
    return p;
  }
}
...
exp_and
    : exp_and ANDAND exp_rel
      {
        auto n = std::make_shared<EBinary>(EBinary::AndAnd, $1, $3);
        n->pos = HERE();
        $$ = n;
      }
    | exp_rel
...
exp_rel
  ...
  | exp_add EQ exp_add
    {
      auto n = std::make_shared<EBinary>(EBinary::Eq, $1, $3);
      n->pos = HERE();
      $$ = n;
    }
  | exp_add NE exp_add
    {
      auto n = std::make_shared<EBinary>(EBinary::Ne, $1, $3);
      n->pos = HERE();
      $$ = n;
    }
  | exp_add LE exp_add
    {
      auto n = std::make_shared<EBinary>(EBinary::Le, $1, $3);
      n->pos = HERE();
      $$ = n;
    }
  | exp_add GE exp_add
    {
      auto n = std::make_shared<EBinary>(EBinary::Ge, $1, $3);
      n->pos = HERE();
      $$ = n;
    }
  | exp_add LT exp_add
    {
      auto n = std::make_shared<EBinary>(EBinary::Lt, $1, $3);
      n->pos = HERE();
      $$ = n;
    }
  | exp_add GT exp_add
    {
      auto n = std::make_shared<EBinary>(EBinary::Gt, $1, $3);
      n->pos = HERE();
      $$ = n;
    }
  ;
...
exp_add
    : exp_add MAIS exp_mul
      {
        auto n = std::make_shared<EBinary>(EBinary::Add, $1, $3);
        n->pos = HERE();
        $$ = n;
      }
    | exp_add MENOS exp_mul
      {
        auto n = std::make_shared<EBinary>(EBinary::Sub, $1, $3);
        n->pos = HERE();
        $$ = n;
      }
    | exp_mul
...
exp_mul
    : exp_mul MULT exp_un
      {
        auto n = std::make_shared<EBinary>(EBinary::Mul, $1, $3);
        n->pos = HERE();
        $$ = n;
      }
    | exp_mul DIVISAO exp_un
      {
        auto n = std::make_shared<EBinary>(EBinary::Div, $1, $3);
        n->pos = HERE();
        $$ = n;
      }
    | exp_mul RESTO exp_un
      {
        auto n = std::make_shared<EBinary>(EBinary::Mod, $1, $3);
        n->pos = HERE();
        $$ = n;
      }
    | exp_un
...
exp_un
    : NEGACAO exp_un
      {
        auto n = std::make_shared<EUnary>(EUnary::Not, $2);
        n->pos = HERE();
        $$ = n;
      }
    | MENOS exp_un %prec UMINUS
      {
        auto n = std::make_shared<EUnary>(EUnary::Neg, $2);
        n->pos = HERE();
        $$ = n;
      }
    | exp_atom
...
exp_atom
    : TRUE_LIT
      {
        auto n = std::make_shared<EBool>(true);
        n->pos = HERE();
        $$ = n;
      }
    | FALSE_LIT
      {
        auto n = std::make_shared<EBool>(false);
        n->pos = HERE();
        $$ = n;
      }
    | NULL_LIT
      {
        auto n = std::make_shared<ENull>();
        n->pos = HERE();
        $$ = n;
      }
    | INT
      {
        auto n = std::make_shared<EInt>($1);
        n->pos = HERE();
        $$ = n;
      }
    | FLOAT
      {
        auto n = std::make_shared<EFloat>($1);
        n->pos = HERE();
        $$ = n;
      }
    | CHAR
      {
        auto n = std::make_shared<EChar>($1);
        n->pos = HERE();
        $$ = n;
      }
    | NEW type new_suffix
      {
        auto n = std::make_shared<ENew>($2, $3);
        n->pos = HERE();
        $$ = n;
      }
    ...
    | lvalue
      {
        auto e = std::make_shared<ELValue>($1);
        e->pos = HERE();
        $$ = e;
      }
...
lvalue
    : ID
      {
        auto v = std::make_shared<LVar>($1);
        v->pos = HERE();
        $$ = v;
      }
    | lvalue DOT ID
      {
        auto f = std::make_shared<LField>($1, $3);
        f->pos = HERE();
        $$ = f;
      }
    | lvalue L_COLCHETE exp R_COLCHETE
      {
        auto i = std::make_shared<LIndex>($1, $3);
        i->pos = HERE();
        $$ = i;
      }
    ;
