...
void TypeChecker::buildDelta(const Program& prog) {
  for (auto& d : prog.decls) {
    ...
    if (delta.count(dd->typeName)) {
      err(SrcPos{}, "Tipo já definido: " + dd->typeName);
    }

    std::unordered_map<std::string, TypePtr> fields;
    for (auto& [fname, fty] : dd->fields) {
      if (fields.count(fname)) {
        err(SrcPos{}, "Campo repetido em " + dd->typeName + ": " + fname);
      }
...
void TypeChecker::buildTheta(const Program& prog) {
  ...
  for (auto& d : prog.decls) {
    ...
    // NÃO deixar função normal ter o mesmo nome de método de class
    if (methodOwner.count(fd->name)) {
      err(SrcPos{}, "Função '" + fd->name + "' conflita com método de classe");
    }

    if (theta.count(fd->name)) {
      err(SrcPos{}, "Função já definida: " + fd->name);
    }

    FunSig sig = parseFunSig(fd->params, fd->typeAnnotText);

    if (!fd->params.empty() && sig.params.size() != fd->params.size()) {
      err(SrcPos{}, "Assinatura não bate com quantidade de parâmetros em " + fd->name);
    }
...
void TypeChecker::requireMainVoid(const Program& /*prog*/) {
  ...
  if (it == theta.end()) {
    err(SrcPos{}, "Programa sem função main");
  }
  auto& sig = it->second;
  if (!sig.params.empty()) err(SrcPos{}, "main não deve receber parâmetros");
  if (sig.rets.size() != 1 || !std::holds_alternative<TyVoid>(sig.rets[0]->v)) {
    err(SrcPos{}, "main deve ter retorno Void");
  }
}
